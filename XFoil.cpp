...
inline void plu3x3(double m[3][3], int piv[3]) {
  ...
  int p = 0;
  if (std::abs(m[1][0]) > std::abs(m[0][0])) p = 1;
  if (std::abs(m[2][0]) > std::abs(m[p][0])) p = 2;
  if (p != 0) {
    std::swap(m[0], m[p]);
...
bool XFoil::blsolve() {
  ...
  auto eliminateVaBlock = [&](int iv, int ivp) {
    ...
    for (int l = iv; l <= nsys; l++) {
      double rhs[3] = {vm[0][l][iv], vm[1][l][iv], vm[2][l][iv]};
      luSolve3x3(D, piv, rhs);
      vm[0][l][iv] = rhs[0];
      vm[1][l][iv] = rhs[1];
      vm[2][l][iv] = rhs[2];
    }

    double rhs0[3] = {vdel[iv](0, 0), vdel[iv](1, 0), vdel[iv](2, 0)};
    luSolve3x3(D, piv, rhs0);
    vdel[iv](0, 0) = rhs0[0];
    vdel[iv](1, 0) = rhs0[1];
    vdel[iv](2, 0) = rhs0[2];

    double rhs1[3] = {vdel[iv](0, 1), vdel[iv](1, 1), vdel[iv](2, 1)};
    luSolve3x3(D, piv, rhs1);
    vdel[iv](0, 1) = rhs1[0];
    vdel[iv](1, 1) = rhs1[1];
    vdel[iv](2, 1) = rhs1[2];
  };
...
bool XFoil::cdcalc() {
  ...
  double uewake =
      uedg.bottom[nbl.bottom] * (1.0 - tklam) / (1.0 - tklam * urat * urat);
  ...
  for (int is = 1; is <= 2; is++) {
    ...
    for (int ibl = 2; ibl <= nbl.get(is); ibl++) {
      ...
      if (ibl == iblte.get(is) + 1) {
        ...
        due1 = 0.0;
        dds1 =
            dte_ute1 * (uedg.top[iblte.top] - usav.top[iblte.top]) +
  ...
  for (int is = 1; is <= 2; ++is) {
    ...
    for (int ibl = 2; ibl <= nbl.get(is); ++ibl) {
      ...
      msq = uedg.get(is)[ibl] * uedg.get(is)[ibl] * hstinv /
            (gamm1 *
             (1.0 - 0.5 * uedg.get(is)[ibl] * uedg.get(is)[ibl] * hstinv));
...
bool XFoil::xyWake() {
  ...
  Vector2d psi = {psilin(n + 1, points.col(n + 1), {1.0, 0.0}, false).psi_ni,
  ...
  for (int i = n + 2; i <= n + nw; i++) {
    ...
    if (i != n + nw) {
      //---- calculate streamfunction gradient components at first point
      Vector2d psi = {psilin(i, points.col(i), {1.0, 0.0}, false).psi_ni,
