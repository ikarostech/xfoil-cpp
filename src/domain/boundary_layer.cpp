...
boundary_layer::DensityShapeParameterResult boundary_layer::hct(double hk,
                                                                double msq) {
...
boundary_layer::KineticShapeParameterResult boundary_layer::hkin(double h,
                                                                 double msq) {
...
boundary_layer::ThicknessShapeParameterResult boundary_layer::hsl(double hk) {
  ...
  if (hk < 4.35) {
    ...
    result.hs_hk =
        ...
        0.0278 *
            (3.0 * MathUtil::pow(tmp, 2) - MathUtil::pow(tmp, 3) / (hk + 1.0)) /
            (hk + 1.0) -
...
boundary_layer::ThicknessShapeParameterResult
boundary_layer::hst(double hk, double rt, double msq) {
  ...
  if (hk < ho) {
    ...
    result.hs =
        (2.0 - hsmin - 4.0 / rtz) * MathUtil::pow(hr, 2) * 1.5 / (hk + 0.5) +
        hsmin + 4.0 / rtz;
    result.hs_hk =
        -(2.0 - hsmin - 4.0 / rtz) * MathUtil::pow(hr, 2) * 1.5 /
            MathUtil::pow(hk + 0.5, 2) +
        (2.0 - hsmin - 4.0 / rtz) * hr * 2.0 * 1.5 / (hk + 0.5) * hr_hk;
    result.hs_rt =
        (2.0 - hsmin - 4.0 / rtz) * hr * 2.0 * 1.5 / (hk + 0.5) * hr_rt +
        (MathUtil::pow(hr, 2) * 1.5 / (hk + 0.5) - 1.0) * 4.0 /
            MathUtil::pow(rtz, 2) * rtz_rt;
  } else {
    //----- separated branch
    ...
    const double htmp_hk =
        -.014 * grt / MathUtil::pow(rtmp, 3) - dhsinf / MathUtil::pow(hk, 2);
    const double htmp_rt =
        -.014 * grt / rtmp / rtmp / rtmp *
    ...
    result.hs_rt = MathUtil::pow(hdif, 2) * htmp_rt -
                   4.0 / MathUtil::pow(rtz, 2) * rtz_rt +
                   hdif * 2.0 * htmp * (-ho_rt);
  }
