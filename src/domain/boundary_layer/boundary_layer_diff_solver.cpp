...
void BlDiffSolver::bldifMomentum(BoundaryLayerState &boundaryLayerState,
  ...
  // Row k=1: build with vector operations (t,d,u columns as a segment)
  {
    ...
    Vector3d cfm1v(skinFriction.cfm_t1, skinFriction.cfm_d1,
                   skinFriction.cfm_u1);
    Vector3d cfm2v(skinFriction.cfm_t2, skinFriction.cfm_d2,
                   skinFriction.cfm_u2);
...
void BlDiffSolver::bldifShape(BoundaryLayerState &boundaryLayerState,
                              double upw, double xlog, double ulog, double hlog,
                              double ddlog, const Vector3d &upw1,
  ...
  double dix = (1.0 - upw) * station1.diz.scalar * xot1 +
               upw * station2.diz.scalar * xot2;
  double cfx = (1.0 - upw) * station1.cfz.scalar * xot1 +
  ...
  // Column t,d,u increments as a vector
  {
    Vector3d inc1 = 0.5 * (z_hca * station1.hcz.pos_vector() +
                           z_ha * Vector3d(station1.param.hz_tz,
                                           station1.param.hz_dz, 0.0)) +
                    z_upw * upw1;
    Vector3d inc2 = 0.5 * (z_hca * station2.hcz.pos_vector() +
                           z_ha * Vector3d(station2.param.hz_tz,
                                           station2.param.hz_dz, 0.0)) +
                    z_upw * upw2;
