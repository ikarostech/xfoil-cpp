...
blData BoundaryLayerVariablesSolver::computeShapeParameters(
  ...
  if (flowRegimeType == FlowRegimeEnum::Laminar) {
    ...
    hs_result = boundary_layer::hst(result.hkz.scalar, result.rtz.scalar,
                                    result.param.mz);
    result.hsz.scalar = hs_result.hs;
  }
...
blData BoundaryLayerVariablesSolver::computeThickness(
    const blData& ref, [[maybe_unused]] FlowRegimeEnum flowRegimeType) const {
  blData result = ref;
