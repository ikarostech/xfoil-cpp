...
Edge::Edge()
    : point_le(Eigen::Vector2d::Zero()), point_te(Eigen::Vector2d::Zero()),
      chord(0.0), sle(0.0), ante(0.0), aste(0.0), dste(0.0), sharp(false) {}

Edge::Edge(const FoilShape &foilShape) : Edge() {

  sle = lefind(foilShape.points, foilShape.dpoints_ds,
               foilShape.spline_length.head(foilShape.n), foilShape.n);

  point_le.x() =
      spline::seval(sle, foilShape.points.row(0), foilShape.dpoints_ds.row(0),
                    foilShape.spline_length.head(foilShape.n), foilShape.n);
  point_le.y() =
      spline::seval(sle, foilShape.points.row(1), foilShape.dpoints_ds.row(1),
                    foilShape.spline_length.head(foilShape.n), foilShape.n);
  point_te =
      0.5 * (foilShape.points.col(0) + foilShape.points.col(foilShape.n - 1));
...
double Edge::lefind(const Eigen::Matrix2Xd &points,
                    ...
                    const Eigen::VectorXd &s, int n) {
  const double dseps = (s[n - 1] - s[0]) * 0.00001;
  const Eigen::Vector2d point_te_local =
      0.5 * (points.col(0) + points.col(n - 1));
