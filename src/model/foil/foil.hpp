...
#include "model/foil/foil_shape.hpp"
...
public:
  ...
  const FoilShape &getFoilShape(int index) const {
    if (0 <= index && index < foil_shape.n) {
      return foil_shape;
    } else if (foil_shape.n <= index && index < foil_shape.n + wake_shape.n) {
      return wake_shape;
    }
    throw std::out_of_range("Foil::getFoilShape: index out of range");
  }

  bool xyWake(int wake_point_count, const Eigen::Matrix2Xd &gamu,
              const Eigen::Matrix2Xd &surface_vortex, double alfa, double qinf);
};
