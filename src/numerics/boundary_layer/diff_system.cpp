...
static LogarithmicDifferences
getLogarithmicDifferences(FlowRegimeEnum flowRegimeType,
...
BlSystemCoeffs BlDiffSolver::solve(FlowRegimeEnum flowRegimeType,
                                   ...
                                   double amcrit) {
  const LogarithmicDifferences logDiffs =
      getLogarithmicDifferences(flowRegimeType, boundaryLayerState);
  ...
  const double hl =
      log(fabs((station2.hkz.scalar - 1.0) / (station1.hkz.scalar - 1.0)));
  ...
  if (flowRegimeType == FlowRegimeEnum::Similarity) {
    ...
    bldifTurbulent(boundaryLayerState, flowRegimeType, upw, upw1, upw2, upw_ms,
                   logDiffs.ulog, coeffs);
  }

  bldifMomentum(boundaryLayerState, logDiffs.xlog, logDiffs.ulog, logDiffs.tlog,
                logDiffs.ddlog, skinFriction, coeffs);
  bldifShape(boundaryLayerState, upw, logDiffs.xlog, logDiffs.ulog,
             logDiffs.hlog, logDiffs.ddlog, upw1, upw2, upw_ms, coeffs);

  return coeffs;
}

void BlDiffSolver::bldifLaminar(BoundaryLayerState &boundaryLayerState,
                                double amcrit, BlSystemCoeffs &coeffs) {
  blData &station1 = boundaryLayerState.station1;
  blData &station2 = boundaryLayerState.station2;

  const auto ax_result = BoundaryLayerUtil::axset(
      station1.hkz.scalar, station1.param.tz, station1.rtz.scalar,
      station1.param.amplz, station2.hkz.scalar, station2.param.tz,
      station2.rtz.scalar, station2.param.amplz, amcrit);

  const double rezc = station2.param.amplz - station1.param.amplz -
  ...
  coeffs.a1(0, 1) =
      z_ax * (ax_result.ax_hk1 * station1.hkz.t() + ax_result.ax_t1 +
  ...
  coeffs.a2(0, 1) =
      z_ax * (ax_result.ax_hk2 * station2.hkz.t() + ax_result.ax_t2 +
...
void BlDiffSolver::bldifTurbulent(BoundaryLayerState &boundaryLayerState,
  ...
  const double rezc = scc * (cqa - sa * ald) * dxi - dea * 2.0 * slog +
                      dea * 2.0 * (uq * dxi - ulog);
  ...
  const double z_upw = z_cqa * (station2.cqz.scalar - station1.cqz.scalar) +
  ...
  coeffs.d_msq[0] = z_upw * upw_ms + z_de * station1.dez.ms() +
                    z_us * station1.usz.ms() + z_de * station2.dez.ms() +
                    z_us * station2.usz.ms() + z_cq1 * station1.cqz.ms() +
                    z_cf1 * station1.cfz.ms() + z_hk1 * station1.hkz.ms() +
                    z_cq2 * station2.cqz.ms() + z_cf2 * station2.cfz.ms() +
...
void BlDiffSolver::bldifMomentum(BoundaryLayerState &boundaryLayerState,
                                 double xlog, double ulog, double tlog,
                                 double ddlog,
                                 const SkinFrictionCoefficients &skinFriction,
                                 BlSystemCoeffs &coeffs) {
  blData &station1 = boundaryLayerState.station1;
  ...
  const double hwa = 0.5 * (station1.param.dwz / station1.param.tz +
  ...
  const double cfx_x1 =
      0.25 * station1.cfz.scalar / station1.param.tz + cfx_xa * 0.5;
  const double cfx_x2 =
      0.25 * station2.cfz.scalar / station2.param.tz + cfx_xa * 0.5;
  const double cfx_t1 =
      -.25 * station1.cfz.scalar * station1.param.xz / station1.param.tz /
          station1.param.tz +
      cfx_ta * 0.5;
  const double cfx_t2 =
      -.25 * station2.cfz.scalar * station2.param.xz / station2.param.tz /
          station2.param.tz +
      cfx_ta * 0.5;
  ...
  const double z_t1 =
      ...
      z_hwa * 0.5 *
          (-station1.param.dwz / station1.param.tz / station1.param.tz);
  const double z_t2 =
      z_tl / station2.param.tz + z_cfx * cfx_t2 +
      z_hwa * 0.5 *
          (-station2.param.dwz / station2.param.tz / station2.param.tz);
...

  {
    ...
    const Vector3d cfm1v(skinFriction.cfm_t1, skinFriction.cfm_d1,
                         skinFriction.cfm_u1);
    const Vector3d cfm2v(skinFriction.cfm_t2, skinFriction.cfm_d2,
                         skinFriction.cfm_u2);
    ...
    const Vector3d seg1 =
        0.5 * z_ha * hterm1 + z_cfm * cfm1v + z_cf1 * cfz1v + mz1 +
        Vector3d(z_t1, 0.0, z_u1);
    const Vector3d seg2 =
        0.5 * z_ha * hterm2 + z_cfm * cfm2v + z_cf2 * cfz2v + mz2 +
  ...
  coeffs.d_msq[1] = 0.5 * z_ma * station1.param.mz_ms +
                    z_cfm * skinFriction.cfm_ms + z_cf1 * station1.cfz.ms() +
                    0.5 * z_ma * station2.param.mz_ms +
                    z_cf2 * station2.cfz.ms();
  coeffs.d_re[1] = z_cfm * skinFriction.cfm_re + z_cf1 * station1.cfz.re() +
                   z_cf2 * station2.cfz.re();
  coeffs.d_xi[1] = 0.0;
  coeffs.rhs[1] = -rezt;
}

void BlDiffSolver::bldifShape(BoundaryLayerState &boundaryLayerState,
                              double upw, double xlog, double ulog,
                              double hlog, double ddlog, const Vector3d &upw1,
                              const Vector3d &upw2, double upw_ms,
                              BlSystemCoeffs &coeffs) {
  blData &station1 = boundaryLayerState.station1;
  blData &station2 = boundaryLayerState.station2;
  ...
  const double dix = (1.0 - upw) * station1.diz.scalar * xot1 +
                     upw * station2.diz.scalar * xot2;
  const double cfx = (1.0 - upw) * station1.cfz.scalar * xot1 +
                     upw * station2.cfz.scalar * xot2;
  ...
  const double z_x2 =
      upw * (z_cfx * station2.cfz.scalar + z_dix * station2.diz.scalar) /
          station2.param.tz +
...

  {
    ...
    const Vector3d base1 =
        z_hs1 * station1.hsz.pos_vector() + z_cf1 * station1.cfz.pos_vector() +
        z_di1 * station1.diz.pos_vector() + Vector3d(z_t1, 0.0, z_u1);
    const Vector3d base2 =
        z_hs2 * station2.hsz.pos_vector() + z_cf2 * station2.cfz.pos_vector() +
  ...
  {
    const Vector3d inc1 =
        0.5 * (z_hca * station1.hcz.pos_vector() +
               z_ha *
                   Vector3d(station1.param.hz_tz, station1.param.hz_dz, 0.0)) +
        z_upw * upw1;
    const Vector3d inc2 =
        0.5 * (z_hca * station2.hcz.pos_vector() +
               z_ha *
                   Vector3d(station2.param.hz_tz, station2.param.hz_dz, 0.0)) +
        z_upw * upw2;
  ...
  coeffs.d_msq[2] =
      0.5 * (z_hca * station1.hcz.ms()) + z_upw * upw_ms +
