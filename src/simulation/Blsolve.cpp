...
namespace {
using Matrix3x2dVector = Blsolve::Matrix3x2dVector;
using VmMatrix = Blsolve::VmMatrix;
using VzMatrix = Blsolve::VzMatrix;
...
std::vector<Eigen::Matrix<double, 3, 2>> Blsolve::solve(int nsys,
  ...
  for (int iv = 1; iv < nsys; ++iv) {
    int ivp = iv + 1;
    eliminateVaBlock(iv, nsys, va, vm, vdel);
    if (iv != nsys - 1) {
      eliminateVbBlock(iv, ivp, nsys, vb, ivte, vz, vm, vdel);
      if (ivp != nsys - 1) {
        eliminateLowerVmColumn(iv, ivp, nsys, vaccel, vm, vdel);
      }
    }
  }

  backSubstitute(nsys, vm, vdel);
  return vdel;
}
