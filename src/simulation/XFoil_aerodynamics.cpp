...
double XFoil::cdcalc() const {
  ...
  const double thwake =
      boundaryLayerWorkflow.lattice.get(2).profiles.momentumThickness
          [boundaryLayerWorkflow.lattice.bottom.stationCount - 2];
  const double edgeVelocity_bottom =
      boundaryLayerWorkflow.lattice.bottom.profiles
          .edgeVelocity[boundaryLayerWorkflow.lattice.bottom.stationCount - 2];
  ...
  const double shwake =
      boundaryLayerWorkflow.lattice.get(2).profiles.displacementThickness
          [boundaryLayerWorkflow.lattice.bottom.stationCount - 2] /
      boundaryLayerWorkflow.lattice.get(2).profiles.momentumThickness
          [boundaryLayerWorkflow.lattice.bottom.stationCount - 2];
...
double XFoil::getXcp() const { return aero_coeffs_.xcp; }
...
XFoil::ClComputation XFoil::clcalc(Vector2d ref) const {
  ...
  const Matrix2d rotateMatrix =
      MathUtil::getRotateMatrix(analysis_state_.alpha);
...
FoilAerodynamicCache XFoil::ggcalc() {
  ...
  MatrixXd dpsi_dgam =
      MatrixXd::Zero(foil.foil_shape.n + 1, foil.foil_shape.n + 1);
  ...
  psi.leftCols(foil.foil_shape.n) = -analysis_state_.qinf *
                                    Matrix2d{{0.0, 1.0}, {-1.0, 0.0}} *
  ...
  cache.bij.row(foil.foil_shape.n).head(foil.foil_shape.n) =
      VectorXd::Zero(foil.foil_shape.n);
...
bool XFoil::specal() { return InviscidSolver::specal(*this); }

bool XFoil::speccl() { return InviscidSolver::speccl(*this); }
