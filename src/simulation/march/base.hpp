...
public:
  ...
  int resetSideState(BoundaryLayerWorkflow &workflow, int side,
  ...
  FlowRegimeEnum
  determineRegimeForStation(const BoundaryLayerWorkflow &workflow, int side,
                            int stationIndex, bool similarity, bool wake) const;
  ...
  computeNewUeDistribution(const BoundaryLayerWorkflow &workflow,
                           const XFoil &xfoil,
...
inline void Marcher::resetStationKinematicsAfterFailure(
  ...
  if (stationIndex <= workflow.lattice.get(side).trailingEdgeIndex) {
    ...
    ctx.thi = workflow.lattice.get(side)
                  .profiles.momentumThickness[stationIndex - 1] *
              scale;
    ctx.dsi = workflow.lattice.get(side)
    ...
    if (stationIndex == workflow.lattice.get(side).trailingEdgeIndex + 1) {
      ...
      const double ratlen =
          (workflow.lattice.get(side).arcLengthCoordinates[stationIndex] -
           workflow.lattice.get(side).arcLengthCoordinates[stationIndex - 1]) /
          (10.0 * workflow.lattice.get(side)
                      .profiles.displacementThickness[stationIndex - 1]);
      ctx.dsi = (workflow.lattice.get(side)
                     .profiles.displacementThickness[stationIndex - 1] +
                 ctx.thi * ratlen) /
  ...
  if (stationIndex > workflow.lattice.get(side).profiles.transitionIndex) {
    ctx.cti =
        workflow.lattice.get(side).profiles.skinFrictionCoeff[stationIndex - 1];
  }
}
