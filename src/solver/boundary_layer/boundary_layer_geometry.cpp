...
BoundaryLayerGeometry::BoundaryLayerGeometry(SidePair<BoundaryLayerLattice> &lattice, Eigen::VectorXd &wgap,
...
bool BoundaryLayerGeometry::iblpan(int point_count, int wake_point_count) {
    ...
    lattice_.bottom.stationCount = lattice_.bottom.trailingEdgeIndex + wake_point_count + 2;
    ...
    for (int iw = 0; iw < wake_point_count; iw++) {
        ...
        lattice_.top.panelInfluenceFactor[lattice_.top.trailingEdgeIndex + iw + 1] = 1.0;
    }
...
bool BoundaryLayerGeometry::stmove(const Eigen::Matrix2Xd &surface_vortex, const Eigen::VectorXd &spline_length,
                                   const Foil &foil, const Eigen::Matrix2Xd &qinv_matrix, StagnationFeature &stagnation,
                                   int &nsys) {
    const int previous           = stagnation_.index();
    const auto stagnation_result = StagnationFeature(surface_vortex, spline_length);
    ...
    if (previous == stagnation_.index()) {
        ...
        if (stagnation_.index() > previous) {
            ...
            const double dudx = lattice_.top.profiles.edgeVelocity[delta] / lattice_.top.arcLengthCoordinates[delta];
            for (int ibl = delta; ibl >= 1; --ibl) {
                copyStationState(1, ibl - 1, delta);
                lattice_.top.profiles.edgeVelocity[ibl - 1] = dudx * lattice_.top.arcLengthCoordinates[ibl - 1];
            }
            ...
            for (int ibl = lattice_.bottom.stationCount - 1; ibl >= delta + 1; --ibl) {
                copyStationState(2, ibl - 1, (ibl - delta) - 1);
            }

            const double dudx =
                lattice_.bottom.profiles.edgeVelocity[delta] / lattice_.bottom.arcLengthCoordinates[delta];
            for (int ibl = delta; ibl >= 1; --ibl) {
                copyStationState(2, ibl - 1, delta);
                lattice_.bottom.profiles.edgeVelocity[ibl - 1] = dudx * lattice_.bottom.arcLengthCoordinates[ibl - 1];
            }
    ...
    for (int is = 1; is <= 2; ++is) {
        for (int ibl = 0; ibl < lattice_.get(is).stationCount - 1; ++ibl) {
            lattice_.get(is).profiles.massFlux[ibl] =
                lattice_.get(is).profiles.displacementThickness[ibl] * lattice_.get(is).profiles.edgeVelocity[ibl];
        }
    }
...
SidePair<Eigen::Matrix2Xd> BoundaryLayerGeometry::uicalc(const Eigen::Matrix2Xd &qinv_matrix) const {
    //--------------------------------------------------------------
    //     sets inviscid ue from panel inviscid tangential velocity
    //--------------------------------------------------------------
    SidePair<Matrix2Xd> inviscid_matrix;
    for (int side = 1; side <= 2; ++side) {
        inviscid_matrix.get(side) = Matrix2Xd::Zero(2, lattice_.get(side).stationCount);
        for (int stationIndex = 0; stationIndex < lattice_.get(side).stationCount - 1; ++stationIndex) {
            const int panelIndex = lattice_.get(side).stationToPanel[stationIndex];
            inviscid_matrix.get(side)(0, stationIndex) =
                lattice_.get(side).panelInfluenceFactor[stationIndex] * qinv_matrix(0, panelIndex);
            inviscid_matrix.get(side)(1, stationIndex) =
                lattice_.get(side).panelInfluenceFactor[stationIndex] * qinv_matrix(1, panelIndex);
        }
    }
...
bool BoundaryLayerGeometry::xicalc(const Foil &foil) {
    ...
    const auto arc_lengths               = computeArcLengthCoordinates(foil, stagnation_.sst(), lattice_);
...
SidePair<VectorXd> BoundaryLayerGeometry::computeArcLengthCoordinates(const Foil &foil, double stagnationSst,
    ...
    for (int ibl = 0; ibl <= lattice.top.trailingEdgeIndex; ++ibl) {
        arc_lengths.top[ibl] = stagnationSst - foil.foil_shape.spline_length[lattice.get(1).stationToPanel[ibl]];
    }

    for (int ibl = 0; ibl <= lattice.bottom.trailingEdgeIndex; ++ibl) {
        arc_lengths.bottom[ibl] = foil.foil_shape.spline_length[lattice.get(2).stationToPanel[ibl]] - stagnationSst;
    }

    // Wake: start from TE, duplicate TE value at first wake station
    arc_lengths.bottom[lattice.bottom.trailingEdgeIndex + 1] = arc_lengths.bottom[lattice.bottom.trailingEdgeIndex];
    for (int ibl = lattice.bottom.trailingEdgeIndex + 2; ibl < lattice.bottom.stationCount; ++ibl) {
        arc_lengths.bottom[ibl] =
            arc_lengths.bottom[ibl - 1] + (foil.wake_shape.points.col(lattice.get(2).stationToPanel[ibl]) -
...
VectorXd BoundaryLayerGeometry::computeWakeGap(const Foil &foil, const BoundaryLayerLattice &bottom,
    ...
    if (foil.edge.sharp) {
        ...
        for (int iw0 = 0; iw0 < foil.wake_shape.n; iw0++) {
            ...
            const double zn     = 1.0 - (bottomArcLengths[te_bot_0b + (iw0 + 1)] - bottomArcLengths[te_bot_0b]) /
...
void BoundaryLayerGeometry::copyStationState(int side, int destination, int source) {
    lattice_.get(side).profiles.skinFrictionCoeff[destination] = lattice_.get(side).profiles.skinFrictionCoeff[source];
    lattice_.get(side).profiles.momentumThickness[destination] = lattice_.get(side).profiles.momentumThickness[source];
    lattice_.get(side).profiles.displacementThickness[destination] =
        lattice_.get(side).profiles.displacementThickness[source];
    lattice_.get(side).profiles.edgeVelocity[destination] = lattice_.get(side).profiles.edgeVelocity[source];
}
