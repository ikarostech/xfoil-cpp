...
class BoundaryLayerSetblAssembler {
  public:
    BoundaryLayerSetblAssembler(BoundaryLayerSetblAccess access, BoundaryLayerMarchAccess march_access)
    ...
    SetblOutputView run(SidePairRef<const BoundaryLayerSideState> profiles, const FlowState &analysis_state,
        ...
        initializeSetblReferenceParams(analysis_state, aero_coeffs, acrit, output, state.re_clmr, state.msq_clmr,
                                       current_mach, current_re);
        ...
        for (int side = 1; side <= 2; ++side) {
            processSetblSide(side, foil, stagnation, analysis_state.controlByAlpha, dij, output, state.stations,
                             state.sides, state.cti, state.ami, state.re_clmr, state.msq_clmr);
        }
  ...
  private:
    ...
    SimilarityStationCoefficients resetSimilarityStationCoefficients(const VectorXd &u_m1, const VectorXd &d_m1) const {
    ...
    StationPrimaryVars loadStationPrimaryVars(int side, int station, bool station_is_wake,
                                              const SetblOutputView &output, double ami, double cti) const {
    ...
    StationUpdateResult updateStationMatricesAndState(int side, int station, int iv, const StationPrimaryVars &vars,
                                                      const SidePair<VectorXd> &usav, const SetblOutputView &output,
                                                      const BoundaryLayerStationWindow &base_state, int system_size,
                                                      const Eigen::MatrixXd &dij) {
        return setbl_station_update::SetblStationUpdateOps::update(access_, side, station, iv, vars, usav,
                                                                   output.profiles, base_state, system_size, dij);
    }

    void buildTransitionLog(bool station_is_transition_candidate, FlowRegimeEnum flow_regime) const {
        if (station_is_transition_candidate && flow_regime != FlowRegimeEnum::Transition) {
            std::stringstream ss;
            ss << "setbl: xtr???  n1=" << access_.previousAmplification() << " n2=" << access_.currentAmplification()
               << ":\n";
            Logger::instance().write(ss.str());
        }
    }

    setbl_te::TeWakeUpdateResult computeTeWakeCoefficients(int side, int station, const SidePair<VectorXd> &usav,
                                                           const SidePair<VectorXd> &ute_m, const SidePair<int> &jvte,
                                                           const VectorXd &d_m1_template, const SetblOutputView &output,
                                                           const Edge &edge) const {
        return setbl_te::SetblTrailingEdgeOps::computeWakeUpdate(access_, side, station, usav, ute_m, jvte,
                                                                 d_m1_template, output.profiles, edge);
    }

    setbl_te::TeWakeJacobianAdjustments
    computeTeWakeJacobianAdjustments(const setbl_te::TeWakeCoefficients &coeffs) const {
        return setbl_te::SetblTrailingEdgeOps::computeJacobianAdjustments(access_, coeffs);
    }

    setbl_edge_velocity::EdgeVelocitySensitivityResult
    prepareEdgeVelocityAndSensitivities(SidePairRef<const BoundaryLayerSideState> profiles, const Eigen::MatrixXd &dij,
                                        int nsys) const {
        return setbl_edge_velocity::SetblEdgeVelocityOps::prepare(access_, profiles, dij, nsys);
    }

    void assembleBlJacobianForStation(int iv, int nsys, const std::array<SetblStation, 2> &setblStations,
        ...
        setbl_jacobian::SetblJacobianOps::assembleForStation(access_, iv, nsys, stations, sideState, controlByAlpha,
                                                             re_clmr, msq_clmr, output.bl_newton_system);
    }

    BoundaryLayerReferenceParams computeBlReferenceParams(const FlowState &analysis_state,
                                                          const AeroCoefficients &aero_coeffs, double acrit) const {
        return BoundaryLayerPhysics::buildReferenceParams(analysis_state, aero_coeffs, acrit);
    }

    void initializeSetblReferenceParams(const FlowState &analysis_state, const AeroCoefficients &aero_coeffs,
                                        double acrit, SetblOutputView &output, double &re_clmr, double &msq_clmr,
                                        double &currentMach, double &currentRe) {
        const auto reference_params   = computeBlReferenceParams(analysis_state, aero_coeffs, acrit);
    ...
    void initializeSetblSystemStorage(SetblOutputView &output, std::array<SetblStation, 2> &stations,
    ...
    void initializeSetblEdgeVelocityState(SidePairRef<const BoundaryLayerSideState> profiles,
                                          const Eigen::MatrixXd &dij, SetblOutputView &output,
                                          SetblSideData &sideData) const {
        const auto edge_result              = prepareEdgeVelocityAndSensitivities(profiles, dij, access_.systemSize());
    ...
    void processSetblSide(int side, const Foil &foil, const StagnationResult &stagnation, bool controlByAlpha,
        ...
        for (int station = 0; station < access_.stationCount(side) - 1; ++station) {
            const int iv                               = access_.stationToSystem(side, station);
            const bool station_is_wake                 = station > access_.trailingEdgeIndex(side);
            const bool station_is_transition_candidate = station == output.profiles.get(side).transitionIndex;

            output.flowRegime    = access_.determineRegimeForStation(side, station);
            access_.flowRegime() = output.flowRegime;

            const auto vars = loadStationPrimaryVars(side, station, station_is_wake, output, ami, cti);
            ami             = vars.ami;
            cti             = vars.cti;

            const auto base_state = access_.snapshotState();
            auto station_update   = updateStationMatricesAndState(side, station, iv, vars, sideData.usav, output,
                                                                  base_state, stations[1].u_m.size(), dij);
            ...
            const auto te_update = computeTeWakeCoefficients(side, station, sideData.usav, sideData.ute_m,
                                                             sideData.jvte, stations[0].d_m, output, foil.edge);
            ...
            output.profiles.get(side).skinFrictionCoeffHistory[station] = access_.currentSkinFrictionHistory();

            setbl_side_sweep::SetblSideSweepOps::assignXiUle(side, stagnation, stations);

            assembleBlJacobianForStation(iv, access_.systemSize(), stations, sideData, controlByAlpha, re_clmr,
                                         msq_clmr, output);

            if (te_update.isStartOfWake) {
                const auto te_jacobian = computeTeWakeJacobianAdjustments(te_update.coeffs);
            ...
            setbl_side_sweep::SetblSideSweepOps::updateRegimeAfterStation(access_, side, station, output);
            setbl_side_sweep::SetblSideSweepOps::advanceStationWindow(stations[1], stations[0], access_);
        }
    }
...
SetblOutputView BoundaryLayerSetblUseCase::run(BoundaryLayer &boundaryLayer,
    ...
    return assembler.run(profiles, analysis_state, aero_coeffs, acrit, foil, stagnation, dij, bl_initialized);
}
