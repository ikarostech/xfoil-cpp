...
class SetblEdgeVelocityOps {
 public:
  static LeTeSensitivities computeLeTeSensitivities(
      const BoundaryLayerSetblAccess &access, int nsys,
    ...
    for (int js = 1; js <= 2; ++js) {
      for (int jbl = 0; jbl < access.stationCount(js) - 1; ++jbl) {
        ...
        sensitivities.ule_m.top[system_index] =
            -access.leadingEdgePanelInfluenceFactor(1) * panel_factor *
            dij(ile1, panel_index);
        sensitivities.ule_m.bottom[system_index] =
            -access.leadingEdgePanelInfluenceFactor(2) * panel_factor *
            dij(ile2, panel_index);
        sensitivities.ute_m.top[system_index] =
            -access.trailingEdgePanelInfluenceFactor(1) * panel_factor *
            dij(ite1, panel_index);
        sensitivities.ute_m.bottom[system_index] =
            -access.trailingEdgePanelInfluenceFactor(2) * panel_factor *
            dij(ite2, panel_index);
      }
    }
  ...
  static EdgeVelocitySensitivityResult prepare(
