...
class SetblStationUpdateOps {
  public:
    static StationUpdateResult update(const BoundaryLayerSetblAccess &access, int side, int station, int iv,
                                      const StationPrimaryVars &vars, const SidePair<Eigen::VectorXd> &usav,
                                      const SidePair<BoundaryLayerSideState> &profiles,
        ...
        for (int js = 1; js <= 2; ++js) {
            for (int jbl = 0; jbl < access.stationCount(js) - 1; ++jbl) {
                ...
                result.u_m2[jv] = -access.panelInfluenceFactor(side, station) * access.panelInfluenceFactor(js, jbl) *
                                  dij(access.stationToPanel(side, station), access.stationToPanel(js, jbl));
        ...
        BoundaryLayerPhysics::refreshCurrentStation(result.state, access.blCompressibility(), access.blReynolds(),
