...
class SetblTrailingEdgeOps {
 public:
  static TeWakeUpdateResult computeWakeUpdate(
      const BoundaryLayerSetblAccess &access, int side, int station,
      ...
      const SidePair<BoundaryLayerSideProfiles> &profiles, const Edge &edge) {
    ...
    result.coeffs.tte =
        profiles.get(1).momentumThickness[top_te] +
        profiles.get(2).momentumThickness[bottom_te];
    result.coeffs.dte = profiles.get(1).displacementThickness[top_te] +
                        profiles.get(2).displacementThickness[bottom_te] +
                        edge.ante;
    result.coeffs.cte =
        (profiles.get(1).skinFrictionCoeff[top_te] *
             profiles.get(1).momentumThickness[top_te] +
         profiles.get(2).skinFrictionCoeff[bottom_te] *
             profiles.get(2).momentumThickness[bottom_te]) /
        result.coeffs.tte;
    ...
    result.coeffs.dte_ute1 =
        -profiles.get(1).displacementThickness[top_te] /
        profiles.top.edgeVelocity[top_te];
    result.coeffs.dte_mte2 = 1.0 / profiles.bottom.edgeVelocity[bottom_te];
    result.coeffs.dte_ute2 =
        -profiles.get(2).displacementThickness[bottom_te] /
        profiles.bottom.edgeVelocity[bottom_te];
    result.coeffs.cte_cte1 =
        profiles.get(1).momentumThickness[top_te] / result.coeffs.tte;
    result.coeffs.cte_cte2 =
        profiles.get(2).momentumThickness[bottom_te] / result.coeffs.tte;
    result.coeffs.cte_tte1 =
        (profiles.get(1).skinFrictionCoeff[top_te] - result.coeffs.cte) /
        result.coeffs.tte;
    result.coeffs.cte_tte2 =
        (profiles.get(2).skinFrictionCoeff[bottom_te] - result.coeffs.cte) /
        result.coeffs.tte;
    ...
    result.dds1 = result.coeffs.dte_ute1 *
                      (profiles.top.edgeVelocity[top_te] - usav.top[top_te]) +
                  result.coeffs.dte_ute2 *
                      (profiles.bottom.edgeVelocity[bottom_te] -
                       usav.bottom[bottom_te]);

    return result;
  }

  static TeWakeJacobianAdjustments
  computeJacobianAdjustments(const BoundaryLayerSetblAccess &access,
