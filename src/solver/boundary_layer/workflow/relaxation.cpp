...
void applyRelaxationLimit(const Eigen::VectorXd &dn, double dhi, double dlo, double &relaxation) {
    double max_pos = 0.0;
    double min_neg = 0.0;
...
BoundaryLayerDelta BoundaryLayerRelaxationOps::buildBoundaryLayerDelta(const BoundaryLayerLattice &lattice_side,
                                                                       const Eigen::VectorXd &unew_side,
    ...
    const Eigen::VectorXd edge_velocity_segment = lattice_side.profiles.edgeVelocity.head(len);
    const Eigen::VectorXd displacement_segment  = lattice_side.profiles.displacementThickness.head(len);

    delta.dedgeVelocity = unew_side.head(len) + dac * u_ac_side.head(len) - edge_velocity_segment;
    delta.ddisplacementThickness =
        (dmass - displacement_segment.cwiseProduct(delta.dedgeVelocity)).cwiseQuotient(edge_velocity_segment);
    return delta;
}

BoundaryLayerMetrics BoundaryLayerRelaxationOps::evaluateSegmentRelaxation(const BoundaryLayerSideState &profiles,
    ...
    const Eigen::VectorXd skin_friction_segment = profiles.skinFrictionCoeff.head(len);
    const Eigen::VectorXd momentum_segment      = profiles.momentumThickness.head(len);
    const Eigen::VectorXd displacement_segment  = profiles.displacementThickness.head(len);
    ...
    const Eigen::VectorXd dn2 = delta.dmomentumThickness.cwiseQuotient(momentum_segment);
    const Eigen::VectorXd dn3 = delta.ddisplacementThickness.cwiseQuotient(displacement_segment);
    ...
    metrics.rmsContribution =
        (dn1.array().square() + dn2.array().square() + dn3.array().square() + dn4.array().square()).sum();
...
BoundaryLayerSideState BoundaryLayerRelaxationOps::applyBoundaryLayerDelta(const BoundaryLayerLattice &lattice_side,
                                                                           const Eigen::VectorXd &wgap,
                                                                           const BoundaryLayerDelta &delta,
                                                                           double relaxation, double hstinv,
                                                                           double gamm1) {
    BoundaryLayerSideState updated = lattice_side.profiles;
    const int len                  = delta.dskinFrictionCoeff.size();
    if (len <= 0) {
        return updated;
    }
    ...
    updated.displacementThickness.head(len) += relaxation * delta.ddisplacementThickness;
    ...
    for (int ibl = 0; ibl < len; ++ibl) {
        ...
        dsw = BoundaryLayerPhysics::adjustDisplacementForHkLimit(dsw, updated.momentumThickness[ibl], msq, hklim);
