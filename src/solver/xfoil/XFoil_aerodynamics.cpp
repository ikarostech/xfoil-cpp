...
double XFoil::cdcalc() const {
    ...
    const double beta = std::sqrt(std::max(0.0, 1.0 - analysis_state_.currentMach * analysis_state_.currentMach));
    const double tklam_local = MathUtil::pow(analysis_state_.currentMach / (1.0 + beta), 2);
    ...
    const double uewake              = edgeVelocity_bottom * (1.0 - tklam_local) / (1.0 - tklam_local * urat * urat);
...
XFoil::ClComputation XFoil::clcalc(Vector2d ref) const {
    ...
    for (int i = 0; i < point_count; i++) {
        ...
        xcp_accumulator +=
            dpoint.x() * ag * (foil.foil_shape.points.col(ip).x() + foil.foil_shape.points.col(i).x()) / 2.0;
...
FoilAerodynamicCache XFoil::ggcalc() {
    ...
    psi.leftCols(foil.foil_shape.n) =
        -analysis_state_.qinf * Matrix2d{{0.0, 1.0}, {-1.0, 0.0}} * foil.foil_shape.points;
    //------ dres/dpsio
