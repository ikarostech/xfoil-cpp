...
class BoundaryLayerViscousUpdate {
 public:
  static ViscousUpdateResult run(const ViscousUpdateInput &input,
                                 const BoundaryLayerMatrix3x2dVector &vdel);

 private:
 static double computeAcChange(double clnew, double cl_current,
                                double cl_target, double cl_ac, double cl_a,
                                double cl_ms, bool controlByAlpha,
                                double currentMach, double machPerLift);
  static double computeRelaxation(const FlowState &analysis_state,
