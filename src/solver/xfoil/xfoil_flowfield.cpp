...
void updateTrailingEdgeState(const Foil &foil,
  ...
  if (state.surfaceVortex.rows() > 0 && state.surfaceVortex.cols() >= node_count) {
    const double surface_delta =
        state.surfaceVortex(0, 0) - state.surfaceVortex(0, node_count - 1);
...
FoilAerodynamicCache buildUnitVorticityDistributions(
  ...
  Eigen::MatrixXd dpsi_dgam =
      Eigen::MatrixXd::Zero(foil.foil_shape.n + 1, foil.foil_shape.n + 1);
  ...
  for (int i = 0; i < foil.foil_shape.n; i++) {
    PsiResult psi_result =
        psilin(foil, i, foil.foil_shape.points.col(i),
               ...
               state.surfaceVortex, analysis_state.alpha, analysis_state.qinf,
  ...
  psi.leftCols(foil.foil_shape.n) =
      -analysis_state.qinf * Eigen::Matrix2d{{0.0, 1.0}, {-1.0, 0.0}} *
      foil.foil_shape.points;
  dpsi_dgam.col(foil.foil_shape.n) = -Eigen::VectorXd::Ones(foil.foil_shape.n);
  dpsi_dgam.row(foil.foil_shape.n) =
      Eigen::VectorXd::Zero(foil.foil_shape.n + 1);
  dpsi_dgam(foil.foil_shape.n, 0) = 1;
  dpsi_dgam(foil.foil_shape.n, foil.foil_shape.n - 1) = 1;

  cache.bij.row(foil.foil_shape.n).head(foil.foil_shape.n) =
      Eigen::VectorXd::Zero(foil.foil_shape.n);

  if (foil.edge.sharp) {
    const double ag1 = atan2(-foil.foil_shape.dpoints_ds.col(0).y(),
                             -foil.foil_shape.dpoints_ds.col(0).x());
    const double ag2 = MathUtil::atanc(
        foil.foil_shape.dpoints_ds.col(foil.foil_shape.n - 1).y(),
    ...
    const double dsmin = std::min(
        ...
        (foil.foil_shape.points.col(foil.foil_shape.n - 1) -
         foil.foil_shape.points.col(foil.foil_shape.n - 2))
            .norm());
    ...
    PsiResult psi_result = psilin(
        foil, -1, bis, normal_bis, true, cache.gamu, state.surfaceVortex,
        analysis_state.alpha, analysis_state.qinf, foil.foil_shape.angle_panel);

    dpsi_dgam.row(foil.foil_shape.n - 1).head(foil.foil_shape.n) =
        psi_result.dzdg;
    cache.bij.row(foil.foil_shape.n - 1).head(foil.foil_shape.n) =
        -psi_result.dzdm;
  ...
  cache.gamu.row(0) =
      cache.psi_gamma_lu.solve(psi.row(0).transpose()).transpose();
  cache.gamu.row(1) =
      cache.psi_gamma_lu.solve(psi.row(1).transpose()).transpose();
  return cache;
}
...
bool buildSourceInfluenceMatrix(const Foil &foil, const FlowState &analysis_state,
  ...
  if (!hasAirfoilInfluenceMatrix()) {
    state.cache.bij.block(0, 0, point_count + 1, point_count) =
        state.cache.psi_gamma_lu
            .solve(state.cache.bij.block(0, 0, point_count + 1, point_count))
            .eval();
    state.cache.dij.block(0, 0, point_count, point_count) =
        state.cache.bij.block(0, 0, point_count, point_count);
  }
  ...
  state.cache.bij.row(point_count)
      .segment(point_count, foil.wake_shape.n)
      .setZero();

  state.cache.bij.block(0, point_count, point_count + 1, foil.wake_shape.n) =
      state.cache.psi_gamma_lu
          .solve(state.cache.bij.block(0, point_count, point_count + 1,
                                       foil.wake_shape.n))
          .eval();
  ...
  for (int i = point_count; i < point_count + foil.wake_shape.n; i++) {
    ...
    PsiResult psi_result = psilin(
        foil, i, foil.wake_shape.points.col(i), foil.wake_shape.normal_vector.col(i),
        true, state.cache.gamu, state.surfaceVortex, analysis_state.alpha,
        analysis_state.qinf, foil.wake_shape.angle_panel);
    cij.row(iw) = psi_result.dqdg.head(point_count).transpose();
    state.cache.dij.row(i).head(point_count) =
        psi_result.dqdm.head(point_count).transpose();

    psi_result = pswlin(foil, i, foil.wake_shape.points.col(i),
                        foil.wake_shape.normal_vector.col(i),
  ...
  state.cache.dij.block(point_count, point_count, foil.wake_shape.n,
                        foil.wake_shape.n) +=
...
Eigen::VectorXd buildViscousTangentialVelocity(
    const BoundaryLayer &boundary_layer, const Eigen::VectorXd &base_qvis) {
  ...
  for (int is = 1; is <= 2; is++) {
    ...
    for (int ibl = 0; ibl < limit; ++ibl) {
      ...
      updated_qvis[i] = boundary_layer.panelInfluenceFactor(is, ibl) *
                        boundary_layer.readStationModel(is, ibl).edgeVelocity;
    }
  }
  return updated_qvis;
}
...
void prepareViscousGeometry(XFoilInternalState &state,
  ...
  foil.xyWake(foil.wake_shape.n, state.inviscid.cache.gamu,
              state.inviscid.surfaceVortex, analysis_state.alpha,
              analysis_state.qinf);
  state.inviscid.qinvu = buildWakeInviscidVelocity(
      foil, state.inviscid.qinvu, state.inviscid.cache.gamu,
      state.inviscid.surfaceVortex, analysis_state.alpha, analysis_state.qinf);
  state.inviscid.qinvMatrix =
      InviscidSolver::qiset(analysis_state.alpha, state.inviscid.qinvu);
...
void assignCurrentInviscidEdgeVelocity(const XFoilInternalState &state,
  ...
  const auto inviscid_edge_velocity =
      boundary_layer.computeInviscidEdgeVelocity(state.inviscid.qinvMatrix);
...
void restoreConvergedOperatingPoint(XFoilResult &result,
  ...
  if (analysis_state.viscous) {
    result.viscousCp = InviscidSolver::cpcalc(total_nodes_with_wake,
                                              state.viscous.qvis,
                                              analysis_state.qinf,
                                              analysis_state.currentMach);
    result.inviscidCp = InviscidSolver::cpcalc(
        total_nodes_with_wake, state.inviscid.qinvMatrix.row(0).transpose(),
        analysis_state.qinf, analysis_state.currentMach);
    return;
  }

  result.inviscidCp = InviscidSolver::cpcalc(
      point_count, state.inviscid.qinvMatrix.row(0).transpose(),
      analysis_state.qinf, analysis_state.currentMach);
}

void refreshViscousFlowFields(XFoilInternalState &state,
                              const FlowState &analysis_state,
                              const Foil &foil,
                              BoundaryLayer &boundary_layer) {
  state.viscous.qvis =
      buildViscousTangentialVelocity(boundary_layer, state.viscous.qvis);
  state.inviscid.surfaceVortex = buildSurfaceVortex(foil, state);
  boundary_layer.moveStagnation(state.inviscid.surfaceVortex,
                                foil.foil_shape.spline_length, foil,
                                state.inviscid.qinvMatrix,
                                state.viscous.stagnation);
}
